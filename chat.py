...
# @st.cache_resource
def initialize_rag_system(chroma_dir: str, collection_name: str, openai_key: str):
    """Initialize the RAG system with specified backend (cached for performance)"""

    try:
        return rag_client.initialize_rag_system(chroma_dir, collection_name, openai_key)
    except Exception as e:
        return None, False, str(e)


def retrieve_documents(
    collection: chromadb.Collection, query: str, n_results: int = 3, mission_filter: str | None = None
) -> dict | None:
...
def evaluate_response_quality(openai_key: str, question: str, answer: str, contexts: list[str]) -> dict[str, float]:
    ...
    try:
        return ragas_evaluator.evaluate_response_quality(openai_key, question, answer, contexts)
    except Exception as e:
...
def get_score_color(score):
    if score >= 0.8:
        return "green"  # Green for high score
    elif score >= 0.6:
        return "orange"  # Orange for medium score
    else:
        return "red"  # Red for low score:
...
def display_evaluation_metrics(scores: dict[str, float]):
    ...
    for metric_name, score in scores.items():
        if isinstance(score, (int, float)):
            color = get_score_color(score=score)
            metric = metric_name.replace("_", " ").title()
            st.sidebar.metric(label=metric, value=f":{color}[{score:.3f}]", delta=None)
            # Add progress bar
...
def main():
    ...
    with st.spinner("Initializing RAG system..."):
        collection, success, error = initialize_rag_system(
            selected_backend["directory"], selected_backend["collection_name"], openai_key
        )
            ...
            with st.spinner("Searching documents and generating response..."):
                ...
                if enable_evaluation and RAGAS_AVAILABLE:
                    with st.spinner("Evaluating response quality..."):
                        evaluation_scores = evaluate_response_quality(openai_key, prompt, response, contexts_list)
